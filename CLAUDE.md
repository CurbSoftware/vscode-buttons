...
**Identity and persistence:** script identity is `scriptKey` = `file:script`; `.buttons.json` stays schema v1 (`script` | `command` entries). For standalone file entries `script` holds the file's path (workspace-relative, or absolute for files outside the workspace) so commands recompute everywhere. Entries for vanished files degrade to `missing` (Run/Copy disabled) rather than being deleted.
