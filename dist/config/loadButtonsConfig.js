...
exports.loadButtonsState = loadButtonsState;
const TOML = __importStar(require("@iarna/toml"));
const vscode = __importStar(require("vscode"));
const findButtonsFile_1 = require("./findButtonsFile");
const DANGER_PATTERNS = [" rm ", " drop ", " prune ", " reset ", " delete ", " deploy "];
const CODICON_PATTERN = /^[a-z][a-z0-9-]*$/;
const HEX_COLOR_PATTERN = /^#(?:[0-9a-fA-F]{6}|[0-9a-fA-F]{3})$/;
async function loadButtonsState(showCommandPreviewFallback, defaultLayout, defaultTerminal) {
...
function resolveDocument(document, showCommandPreviewFallback, defaultLayout, defaultTerminal, diagnostics) {
...
function resolveStaticButtons(groupId, group, document, variables, macros, diagnostics) {
...
function resolveGeneratedButtons(groupId, group, document, variables, macros, diagnostics) {
    ...
    return combinations.flatMap((args, index) => {
        const context = { base: group.base, variables, macros, args };
...
function resolveButton(groupId, group, document, button, command) {
...
function resolveTerminalMode(defaults, group, button, fallback) {
...
function buildCommand(button, group, context) {
...
function expandMacros(input, diagnostics) {
    const resolved = {};
    ...
    const visit = (name) => {
        ...
        const expanded = value.replace(/\{\{\s*([^}]+)\s*\}\}/g, (_match, token) => {
...
function cartesian(params) {
    ...
    return params.reduce((accumulator, values) => {
        const next = [];
...
function validateButton(button, scope, diagnostics) {
...
function validateGenerate(generate, scope, diagnostics) {
...
function validateLinks(links, scope, diagnostics) {
...
function validateUrls(urls, scope, diagnostics) {
...
function validatePorts(ports, scope, diagnostics) {
...
//# sourceMappingURL=loadButtonsConfig.js.map