...
function makeDefaultDisplay(overrides?: Partial<ResolvedGroupDisplay>): ResolvedGroupDisplay {
  return {
    layout: "grid",
...
describe("validateDocument", () => {
  ...
  it("emits deprecation warning for document display block", () => {
    const diagnostics = validateDocument(minimalDoc({
      display: { show_command: true },
    }));
    assert.ok(diagnostics.some((d) => d.severity === "warning" && d.message.includes("deprecated")));
  });

  it("emits deprecation warning for group display block", () => {
    const diagnostics = validateDocument(minimalDoc({
      groups: {
        main: {
...
describe("resolveDocument", () => {
  function resolve(doc: ButtonsDocument, diagnostics?: ButtonsDiagnostic[]) {
    ...
    return resolveDocument(doc, makeDefaultDisplay(), "current", diag);
  }
  ...
  it("uses display defaults from settings", () => {
    const doc: ButtonsDocument = {
      version: 1,
      groups: { main: { buttons: [{ id: "test", command: "echo" }] } },
    };
    const display = makeDefaultDisplay({ compact: true, showRun: false });
    const result = resolveDocument(doc, display, "current", []);
    assert.strictEqual(result.compact, true);
    assert.strictEqual(result.showRun, false);
    assert.strictEqual(result.groups[0].compact, true);
    assert.strictEqual(result.groups[0].showRun, false);
  });

  it("group layout overrides display default layout", () => {
    const doc: ButtonsDocument = {
      version: 1,
      groups: {
        main: {
    ...
    const result = resolveDocument(doc, makeDefaultDisplay(), "current", []);
    assert.strictEqual(result.layout, "grid");
