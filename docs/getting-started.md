...
On a fresh project, the **Project scripts** tab shows every discovered script with its checkboxes disabled, plus a **Generate buttons file** button.

Click **Generate**. Buttons scans the workspace and writes `<workspace root>/.buttons.json` containing every discovered script.
