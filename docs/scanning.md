...
The list is stored in the [`buttons.scanDirectories`](configuration.md#buttonsscandirectories) workspace setting, so it is shared with the project. **Add folder** opens your OS folder picker; removing a directory stops scanning it. Hidden directories and ignore-listed names (like `build` or `node_modules`) cannot be scan targets - they are excluded from every scan and from the file watcher.
